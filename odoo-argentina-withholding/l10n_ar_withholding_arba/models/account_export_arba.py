...
class AccountExportArba(models.Model):
    ...
    export_arba_data_ret = fields.Text('Contenidos archivo ARBA Ret', default='')
    export_arba_data_per = fields.Text('Contenidos archivo ARBA Per', default='')
    tax_withholding = fields.Many2one('account.tax','Imp. de ret utilizado', domain=[('tax_arba_ret', '=', True)], required=True)
    
    #Retenciones
    @api.depends('export_arba_data_ret')
    def _compute_files_ret(self):
        self.ensure_one()
        self.export_arba_filename_ret = _('Arba_ret_%s_%s.txt') % (str(self.date_from),str(self.date_to))
        self.export_arba_file_ret = base64.encodestring(self.export_arba_data_ret.encode('ISO-8859-1'))
    export_arba_file_ret = fields.Binary('TXT ARBA Ret',compute=_compute_files_ret)
    export_arba_filename_ret = fields.Char('TXT ARBA Ret',compute=_compute_files_ret) 
    
    #Percepciones
    @api.depends('export_arba_data_per')
    def _compute_files_per(self):
        self.ensure_one()
        self.export_arba_filename_per = _('Arba_per_%s_%s.txt') % (str(self.date_from),str(self.date_to))
        self.export_arba_file_per = base64.encodestring(self.export_arba_data_per.encode('ISO-8859-1'))
    export_arba_file_per = fields.Binary('TXT ARBA Per',compute=_compute_files_per)
    export_arba_filename_per = fields.Char('TXT ARBA Per',compute=_compute_files_per)
    ...
    def compute_arba_data(self):
        ...
        for payment in payments:
            ...
            if payment.tax_withholding_id.id != self.tax_withholding.id:
                continue
            # TXT segun formato de https://www.arba.gov.ar/Archivos/Publicaciones/dise%C3%B1o_de_registros_bancos.pdf
        ...
        self.export_arba_data_ret = string
        ...
        self.export_arba_data_per = string
