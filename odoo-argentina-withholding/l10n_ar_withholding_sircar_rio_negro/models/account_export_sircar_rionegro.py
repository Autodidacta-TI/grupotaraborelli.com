...
class AccountExportSircarRioNegro(models.Model):
    _name = 'account.export.sircar.rionegro'
    _description = 'account.export.sircar.rionegro'

    name = fields.Char('Nombre')
    date_from = fields.Date('Fecha desde')
    date_to = fields.Date('Fecha hasta')
    export_sircar_rionegro_data_ret = fields.Text('Contenidos archivo SIRCAR Rio Negro Ret', default='')
    export_sircar_rionegro_data_per = fields.Text('Contenidos archivo SIRCAR Rio Negro Per', default='')
    tax_withholding = fields.Many2one('account.tax','Imp. de ret utilizado', domain=[('tax_sircar_rionegro_ret', '=', True)], required=True)
    
    #Retenciones
    @api.depends('export_sircar_rionegro_data_ret')
    def _compute_files_ret(self):
        self.ensure_one()
        self.export_sircar_rionegro_filename_ret = _('Sircar_rionegro_ret_%s_%s.txt') % (str(self.date_from),str(self.date_to))
        self.export_sircar_rionegro_file_ret = encodebytes(self.export_sircar_rionegro_data_ret.encode('ISO-8859-1'))
    export_sircar_rionegro_file_ret = fields.Binary('TXT SIRCAR Rio Negro Ret',compute=_compute_files_ret)
    export_sircar_rionegro_filename_ret = fields.Char('TXT SIRCAR Rio Negro Ret',compute=_compute_files_ret) 
    
    #Percepciones
    @api.depends('export_sircar_rionegro_data_per')
    def _compute_files_per(self):
        self.ensure_one()
        self.export_sircar_rionegro_filename_per = _('Sircar_rionegro_per_%s_%s.txt') % (str(self.date_from),str(self.date_to))
        self.export_sircar_rionegro_file_per = encodebytes(self.export_sircar_rionegro_data_per.encode('ISO-8859-1'))
    export_sircar_rionegro_file_per = fields.Binary('TXT SIRCAR Rio Negro Per',compute=_compute_files_per)
    export_sircar_rionegro_filename_per = fields.Char('TXT SIRCAR Rio Negro Per',compute=_compute_files_per)


    def compute_sircar_rionegro_data(self):
        self.ensure_one()
        ...
        for payment in payments:
            ...
            _alicuota_ret = payment.partner_id.alicuot_ret_sircar_rionegro_ids.filtered(lambda l: l.padron_activo == True)[0].a_ret
        ...
        self.export_sircar_rionegro_data_ret = string
        ...
        for invoice in invoices:
            for group in invoice.amount_by_group:
                if group[0] == 'Perc IIBB Río Negro':
                    _alicuota = invoice.partner_id.alicuot_per_sircar_rionegro_ids.filtered(lambda l: l.padron_activo == True)[0].a_per
        ...
        self.export_sircar_rionegro_data_per = string
