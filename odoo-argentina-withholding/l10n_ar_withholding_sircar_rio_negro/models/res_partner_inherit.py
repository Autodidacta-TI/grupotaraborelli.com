...
class ResPartner(models.Model):
    ...
    alicuot_ret_sircar_rionegro_ids = fields.One2many(
        'partner.padron.sircar.rionegro.ret',
        'partner_id',
        'Alicuotas Retencion',
    )
    alicuot_per_sircar_rionegro_ids = fields.One2many(
        'partner.padron.sircar.rionegro.per',
        'partner_id',
        'Alicuotas Percepcion',
    )

class ResPartnerAlicuotRet(models.Model):
    _name = 'partner.padron.sircar.rionegro.ret'
    ...
    def create(self, vals):
        ...
        for alicuota in parent.alicuot_ret_sircar_rionegro_ids:
...
class ResPartnerAlicuotPer(models.Model):
    _name = 'partner.padron.sircar.rionegro.per'
    _order = 'create_date desc'
    ...
    def create(self, vals):
        parent = self.env['res.partner'].search([('id','=',int(vals['partner_id'])),('parent_id','=',False)],limit=1)
        for alicuota in parent.alicuot_per_sircar_rionegro_ids:
            if alicuota.padron_activo == True:
                alicuota.padron_activo = False
